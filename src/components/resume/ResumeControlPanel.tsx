...
import { Button, Select, MenuItem, FormControl, InputLabel, Divider } from '@mui/material';
...
export default function ResumeControlPanel({ onGenerate, isGenerating }: ResumeControlPanelProps) {
    ...
    useEffect(() => {
        ...
        fetchJobProfiles();
    }, [isSetupOpen]); // Re-fetch when setup closes (potentially new profile created)
            ...
            <GlassCard className="h-full flex flex-col p-6 space-y-6">
                ...
                <div className="space-y-2">
                    <FormControl fullWidth>
                        <InputLabel id="resume-profile-label" sx={{ color: 'rgba(255,255,255,0.7)' }}>Select Base Resume Profile</InputLabel>
                        <Select
                            labelId="resume-profile-label"
                            value={selectedProfile}
                            label="Select Base Resume Profile"
                        ...
                        >
                            {isLoadingProfiles ? (
                                ...
                                [
                                    ...(profileOptions.length === 0 ? [<MenuItem key="none" disabled>No profiles found</MenuItem>] : []),
                                    ...profileOptions.map((option) => (
                                        <MenuItem key={option.value} value={option.value}>
                                            {option.label}
                                        </MenuItem>
                                    )),
                                    <Divider key="divider" light sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />,
